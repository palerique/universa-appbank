...
public class UtilHelper {
	...
	public static boolean isLoginValido(String login) {
		...
		return (login.matches("^([0-9a-zA-Z]+([_.-]?[0-9a-zA-Z]+)*@[0-9a-zA-Z]+[0-9,a-z,A-Z,.,-]*(.){1}[a-zA-Z]{2,4})+$"));
	}

	public static boolean isCpfValido(String cpf) {

		int indice, soma1, soma2, digito1, digito2;
		...
		for (indice = 0; indice <= 8; indice++)
			soma1 = soma1 + Integer.parseInt(cpf.substring(indice, indice + 1)) * (10 - indice);
		...
		for (indice = 0; indice <= 9; indice++)
			soma2 = soma2 + Integer.parseInt(cpf.substring(indice, indice + 1)) * (11 - indice);
